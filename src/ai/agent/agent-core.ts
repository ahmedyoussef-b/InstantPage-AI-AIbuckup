...
export async function processAgentMission(request: string, userId: string): Promise<AgentResponse> {
  console.log(`[AGENT][CORE] Démarrage mission Elite : "${request.substring(0, 40)}..."`);

  try {
    // 1. PHASE 1: COMPRENDRE - Collecte du contexte et analyse de l'intention (MCP)
    console.log(`[AGENT][PHASE-1] Collecte du contexte MCP...`);
    ...
    console.log(`[AGENT][PHASE-2] Décomposition en étapes atomiques...`);
    const steps = await decomposeIntention(intention, context);
    const plan = await createTaskPlan(steps, context);
    console.log(`[AGENT][PHASE-2] Plan d'exécution généré (${plan.steps.length} étapes).`);

    // 3. PHASE 3: AGIR - Exécution du plan via les outils sécurisés MCP
    console.log(`[AGENT][PHASE-3] Lancement de l'exécution séquentielle...`);
    ...
    console.log(`[AGENT][PHASE-4] Consolidation de l'apprentissage...`);
    ...
    console.log(`[AGENT][PHASE-4] Patterns enregistrés dans la base vectorielle.`);
    ...
    console.error("[AGENT][ERROR] Échec critique de la mission :", error);
