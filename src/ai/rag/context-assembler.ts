...
export class ContextAssembler {
  ...
  async assemble(retrievalResult: RetrievalResult): Promise<AssembledContext> {
    ...
    // 2. Limiter la taille du contexte pour respecter la fenêtre de tokens (Memory Management)
    ...
    return {
      ...
      sources: limited.map(c => ({
        source: c.source,
        title: c.metadata?.title || this.getDefaultTitle(c.source),
  ...
  private limitContextSize(contexts: FusedResult[]): FusedResult[] {
    let totalTokens = 0;
    const limit = 3000; // Limite de sécurité pour modèles type Phi3/TinyLlama
    ...
    for (const ctx of contexts) {
      ...
      if (totalTokens + tokens <= limit) {
        ...
        // On s'arrête dès que la fenêtre est pleine pour éviter la troncature sauvage
        break;
      }
    }
  ...
  private formatForLLM(contexts: FusedResult[]): string {
  ...
  private addMetadata(text: string, retrievalResult: RetrievalResult): string {
    ...
    // Adaptation des instructions selon le type de requête (Innovation 32)
  ...
  private countTokens(text: string): number {
    // Heuristique simple pour le français : 1 token ~= 4 caractères
    return Math.ceil(text.length / 4);
  }

  private getDefaultTitle(source: string): string {
    switch(source) {
